...
do_toggle_switch :: proc(value: ^bool, loc := #caller_location) {
	using opal
	assert(value != nil)
	node := do_node(
		&{
			...
			on_draw = proc(self: ^Node) {
				...
				kn.add_box(
					...
					paint = kn.fade(tw.BLUE_500, self.transitions[0]),
				)
				kn.add_box(thumb_box, thumb_size / 2, paint = tw.WHITE)
			},
		},
	)
	if node.is_hovered && node.was_active && !node.is_active {
		value^ = !value^
	}
}
